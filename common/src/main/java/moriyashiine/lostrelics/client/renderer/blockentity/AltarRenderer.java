...
public record AltarRenderer(ItemModelResolver itemModelResolver) implements BlockEntityRenderer<AltarBlockEntity, AltarRenderState> {
	...
	public void extractRenderState(AltarBlockEntity blockEntity, AltarRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.@Nullable CrumblingOverlay breakProgress) {
		...
		state.rotation = Mth.wrapDegrees((state.blockPos.hashCode() + AltarClientEvent.clientTime + partialTicks) * 4);
	}
}
