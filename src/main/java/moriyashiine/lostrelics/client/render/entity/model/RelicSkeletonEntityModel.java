...
public class RelicSkeletonEntityModel extends BipedEntityModel<RelicSkeletonEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		root.addChild(EntityModelPartNames.BODY, ModelPartBuilder.create().uv(0, 45).cuboid(-4, 0, -2, 8, 8, 4, new Dilation(-0.01F))
				.uv(55, 0).cuboid(-1, -1.75F, 1, 2, 12, 1, Dilation.NONE)
				.uv(30, 0).cuboid(-4, 9, -2, 8, 3, 4, new Dilation(-0.01F)), ModelTransform.NONE);
		root.addChild(EntityModelPartNames.LEFT_ARM, ModelPartBuilder.create().uv(0, 29).cuboid(-1, -2, -1, 2, 12, 2, Dilation.NONE), ModelTransform.origin(5, 2, 0));
		root.addChild(EntityModelPartNames.RIGHT_ARM, ModelPartBuilder.create().uv(0, 29).mirrored().cuboid(-1, -2, -1, 2, 12, 2, Dilation.NONE), ModelTransform.origin(-5, 2, 0));
		root.addChild(EntityModelPartNames.LEFT_LEG, ModelPartBuilder.create().uv(12, 29).cuboid(-1, 0, -1, 2, 12, 2, Dilation.NONE), ModelTransform.origin(1.9F, 12, 0));
		root.addChild(EntityModelPartNames.RIGHT_LEG, ModelPartBuilder.create().uv(12, 29).mirrored().cuboid(-1, 0, -1, 2, 12, 2, Dilation.NONE), ModelTransform.origin(-1.9F, 12, 0));
