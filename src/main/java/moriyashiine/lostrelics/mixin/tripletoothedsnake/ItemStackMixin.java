...
public abstract class ItemStackMixin implements ComponentHolder {
	...
	private Text lostrelics$tripleToothedSnake(Text original) {
		if (contains(ModComponentTypes.TAINTED_POTION)) {
			if (isOf(Items.POTION) || isOf(Items.SPLASH_POTION) || isOf(Items.LINGERING_POTION) || isOf(Items.TIPPED_ARROW)) {
				return Text.translatable((getItem().getTranslationKey() + ".lostrelics.tainted_potion").replace(".effect.empty", ""));
			}
		}
		return original;
	}
